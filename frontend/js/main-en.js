...
class LoadingManager {
    ...
    static hide(element) {
        ...
        element.textContent = element.dataset.originalText || 'Send';
...
async function fetchData(endpoint) {
    try {
        ...
        toast.show('Error', 'An error occurred while loading data.', 'error');
        return null;
    }
}
...
function createServiceCard(service) {
    ...
    card.innerHTML = `
        ...
        <a href="practice-areas-en.html#service-${service.id}" class="read-more">
            Read More
        </a>
    `;
    return card;
...
function createPostCard(post) {
    ...
    card.innerHTML = `
        <div class="post-card-content">
            ...
            <a href="blog-en.html#post-${post.id}" class="read-more">
                Read More
            </a>
        </div>
    `;
...
async function handleContactFormSubmit(event) {
    ...
    try {
        ...
        toast.show('Success', result.message || 'Your message has been sent successfully! We will get back to you as soon as possible.', 'success');

    } catch (error) {
        console.error('Contact form error:', error);
        toast.show('Error', 'An error occurred while sending the message. Please try again.', 'error');
    } finally {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const isHomepage = window.location.pathname === '/' ||
        ...
        window.location.pathname === '/c:/Users/Pc/Desktop/polatlar/frontend/index-en.html';
...
async function showServiceDetail(serviceId) {
    try {
        ...
        const content = `
            ...
            <div class="service-contact">
                <h3>Do You Need Legal Support on This Matter?</h3>
                <p>Contact our expert lawyers immediately.</p>
                <a href="iletisim.html" class="btn">
